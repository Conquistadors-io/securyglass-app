  ...
  const calculatePrice = async () => {
    try {
      ...
      const {
        data: result,
        error
      } = await supabase.functions.invoke('compute-quote', {
        body: {
  ...
  const checkGmailConfiguration = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from('gmail_credentials').select('user_email, expires_at').gte('expires_at', new Date().toISOString()).single();
      if (data && !error) {
  ...
  const sendQuoteEmailViaSendGrid = async () => {
    ...
    try {
      ...
      const quoteData = {
        ...
        items: [...(priceCalculation?.details?.deplacement?.total > 0 ? [{
  ...
  return <div className="space-y-6">
      ...
      <Card className="shadow-card border-2 border-blue-500">
        <div className="p-6">
          ...
          {calculationLoading ? <div className="text-center py-4">
              <p className="text-muted-foreground">Calcul en cours...</p>
            </div> : !priceCalculation ? <div className="text-center py-4">
